...
public class TelaCalculadora extends JFrame {
	 ...
	 * Create the frame.
	 */
	public TelaCalculadora(Usuario usuario) {
		setUndecorated(true);
		setResizable(false);
		...
		btnCalcular.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (altura != 0 && peso != 0 && idade != 0 && rdbtnMasculino.isSelected() || rdbtnFeminino.isSelected()
						&& (rdbtnSedentario.isSelected() || rdbtnModerado.isSelected() || rdbtnAtivo.isSelected())) {
					if (rdbtnMasculino.isSelected()) {
						if (rdbtnSedentario.isSelected()) {
							...
							usuario.CalculoHomem(altura, peso, idade,
									ENivelDeAtividadeFisica.MODERADAMENTE_ATIVO.getFator());
						} else if (rdbtnAtivo.isSelected()) {
							usuario.CalculoHomem(altura, peso, idade,
									ENivelDeAtividadeFisica.EXTREMAMENTE_ATIVO.getFator());
						}
					} else if (rdbtnFeminino.isSelected()) {
						if (rdbtnSedentario.isSelected()) {
							usuario.CalculoMulher(altura, peso, idade, ENivelDeAtividadeFisica.SEDENTARIO.getFator());
						} else if (rdbtnModerado.isSelected()) {
							usuario.CalculoMulher(altura, peso, idade,
									ENivelDeAtividadeFisica.MODERADAMENTE_ATIVO.getFator());
						} else if (rdbtnAtivo.isSelected()) {
							usuario.CalculoMulher(altura, peso, idade,
									ENivelDeAtividadeFisica.EXTREMAMENTE_ATIVO.getFator());
		...
		lblBtnHome.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				TelaHome th = new TelaHome(usuario);
				th.setVisible(true);
				dispose();
			}
		});
